               ...
               <div key={hour} className="grid grid-cols-[60px_1fr] border-b border-slate-100 dark:border-slate-800 min-h-[60px]">
                  ...
                  <div className="relative p-1">
                     {dayEvents.filter(e => parseInt(e.startTime.split(':')[0]) === hour).map(event => (
                       <div key={event.id} onClick={(e) => { e.stopPropagation(); setSelectedEvent(event); }} className={`absolute inset-x-1 p-2 rounded-md text-xs border shadow-sm cursor-pointer z-10 ${getEventTypeColor(event.type)}`} style={{ top: `${(parseInt(event.startTime.split(':')[1]) / 60) * 100}%`, minHeight: '30px' }}>
                          <div className="font-bold flex items-center justify-between"><span>{event.title}</span>{event.momSent && <CheckSquare size={10} />}</div>
                       ...
                       return (
                         <div key={i} className="border-b border-l border-slate-100 dark:border-slate-800 min-h-[60px] relative hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors cursor-pointer" onClick={() => setSelectedDate(day)}>
                            {eventsForSlot.map(event => (
                               <div key={event.id} onClick={(e) => { e.stopPropagation(); setSelectedEvent(event); }} className={`absolute mx-1 left-0 right-0 p-1.5 rounded text-[10px] font-medium border truncate shadow-sm z-10 ${getEventTypeColor(event.type)}`} title={event.title}><span className="flex items-center gap-1">{event.momSent && <CheckSquare size={8} />} {event.title}</span></div>
                            ))}
                         </div>
                       );
                    })}
